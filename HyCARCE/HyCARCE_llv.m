...
% each update
%=======================================================
function [matA centers clusterindex dCountHistory datacount] = HyCARCE_llv(inpData,D,gridcellsize,EStep)
%% Initial Boxes

InitBox=ones(1,D)*gridcellsize;
% EStep = 0.97;
%============== Grid Initialization =================
[matA centers datacount EllipseVols] = BoxGrid_ori(inpData,InitBox,D);
%===================================================
 ...
 while(stopFlag>0)
   ...
   while i<=NoOfBox
    ...
    if ( stoppedellipses(i,1)==1)
        [NewElli C dcount vol removable] = EnlargeEllipse_ori(squeeze(matA(i,:,:)),centers(i,:),datacount(i,1),inpData,EStep,AvgSize,EllipseVols(i,1),gridcellsize);
        if(removable==1)
